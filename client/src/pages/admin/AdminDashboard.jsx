...
import MediaSelector from '../../components/course/MediaSelector';
import RichTextEditor from '../../components/common/RichTextEditor';
import { courseAPI, userAPI, settingsAPI } from '../../services/api';
...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [mediaRes, coursesRes, coachesRes, clientsRes, timerRes] = await Promise.all([
        axios.get(`${API_URL}/media`),
        courseAPI.getCourses(),
        userAPI.getCoaches({ limit: 100 }),
        userAPI.getClients(),
        settingsAPI.getSettings('countdown_timer')
      ]);
      setMedia(mediaRes.data);
      setCourses(coursesRes.data.courses || coursesRes.data);
      setCoaches(coachesRes.data.coaches || coachesRes.data);
      ...
      if (timerRes.data && timerRes.data.value) {
        const val = timerRes.data.value;
        setTimerSettings({
          eventName: val.eventName,
          targetDate: new Date(val.targetDate).toISOString().slice(0, 16),
...
toast.success(action === 'add' ? 'Student enrolled' : 'Student unenrolled');
  ...
  const handleUpdateTimerSettings = async (e) => {
    ...
    try {
      await settingsAPI.updateSettings('countdown_timer', {
        ...timerSettings,
        targetDate: new Date(timerSettings.targetDate)
      });
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <div className="bg-white dark:bg-dark-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-dark-700">
              ...
              <form onSubmit={handleUpdateTimerSettings} className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Event Name</label>
                  <input
                    type="text"
                    value={timerSettings.eventName}
                    onChange={(e) => setTimerSettings({ ...timerSettings, eventName: e.target.value })}
                    className="w-full p-2 border border-gray-300 dark:border-dark-700 rounded-lg text-sm bg-white dark:bg-dark-900 text-gray-900 dark:text-gray-100"
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Target Date & Time</label>
                  <input
                    type="datetime-local"
                    value={timerSettings.targetDate}
                    onChange={(e) => setTimerSettings({ ...timerSettings, targetDate: e.target.value })}
                    className="w-full p-2 border border-gray-300 dark:border-dark-700 rounded-lg text-sm bg-white dark:bg-dark-900 text-gray-900 dark:text-gray-100"
                    required
                  />
                </div>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => setTimerSettings({ ...timerSettings, isActive: e.target.checked })}
                    className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                  />
                  <label htmlFor="timerActive" className="text-sm text-gray-700 dark:text-gray-300">Show Timer</label>
                </div>
                <button
                  type="submit"
                  ...
                  className="w-full bg-indigo-600 text-white py-2 rounded-lg font-medium hover:bg-indigo-700 disabled:bg-gray-300 dark:disabled:bg-dark-700 transition-colors"
                >
                  {updatingTimer ? 'Updating...' : 'Update Timer'}
                </button>
              </form>
            </div>
